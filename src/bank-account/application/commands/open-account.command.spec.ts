...
import { OpenAccountCommandHandler, OpenAccountCommand } from './open-account.command';
...
describe('OpenAccountCommandHandler', () => {
    ...
    it('should open a new bank account', async () => {
        ...
        const mergedAggregate = eventPublisher.mergeObjectContext.mock.results[0].value;
    ...
    it('should create account with correct properties', async () => {
        ...
        const callArg = eventPublisher.mergeObjectContext.mock.calls[0][0];
        expect(callArg).toBeInstanceOf(BankAccount);
        expect(callArg.id.toString()).toBe('acc-456');
    ...
    it('should handle multiple account creations', async () => {
        const command1 = new OpenAccountCommand('acc-1', 'User 1', 100, 'USD');
        const command2 = new OpenAccountCommand('acc-2', 'User 2', 200, 'USD');
